//! digg — a modern DNS lookup library and CLI.
//!
//! The wire-format protocol layer (`protocol`) is public so tools and fuzz
//! targets can decode DNS messages directly; the rest supports the binary.
...
/// Print an error to stderr with the binary's formatting (used by `main`).
pub fn eprint_error(msg: &str) {
    output::eprint_error(msg);
}

pub fn run(args: &[String]) -> Result<i32, DnsError> {
